...
The subpackage also defines **conversion strategies** that translate between `VariableDescriptor` objects and external string representations (e.g., Fluent variable names).
...
A registry of predefined `VariableDescriptor` instances for commonly used quantities. These are typically accessed directly by name, e.g., `VariableCatalog.PRESSURE`.
...
.. code-block:: python
    ...
    print(strategy.to_string(descriptor))  # e.g., "SV_VELOCITY"
