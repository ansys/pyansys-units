...
def test_custom_unit_system():
    us = q.UnitSystem(name="sys", base_units=["slug", "ft", "s", "R", "radian", "slugmol", "cd", "A", "sr"])
    assert us.name == "sys"
...
def test_conversion():
    ...
    us2 = q.UnitSystem(name="sys", base_units=["kg", "m", "s", "K", "radian", "mol", "cd", "A", "sr"])
...
def test_errors():
    ...
    with pytest.raises(q.UnitSystemError) as e_info:
        us2 = q.UnitSystem(
            name="sys", 
            base_units=["slug", "ft", "s", "R", "radian", "slugmol", "cd", "A", "sr", "cd", "A", "sr"]
        )
